...
int main(int argnum, char* arg[]) {
  ...
  nowy_lex();
  if (form0(&drz)) {
	Boolean czy_tautologia = TRUE;
	for (int n=0; n<1024; n++) {
		if (wylicz(drz, tab) == '0') {
			czy_tautologia = FALSE;
			break;
		}
		else {
			int przeniesienie = 0;
			char *zmienne = &tab[0] + ('p' - 'a');
			for (int m=0; m<10; m++) {
				zmienne[m] += przeniesienie + 1;
				if (zmienne[m] == '2') {
					zmienne[m] = '0';
					przeniesienie = 1;
	...
	char *ciag = (czy_tautologia == TRUE ? "jest" : "nie jest");
	printf("\n formula %s tautologia\n", ciag);
    //printf("\n  wartosc: %c\n\n", wylicz(drz, tab));
  }
